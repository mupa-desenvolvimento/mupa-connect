CREATE OR REPLACE FUNCTION public.get_dispositivo_por_serial(p_serial text, p_apelido_interno text DEFAULT 'aguardando_ativação'::text, p_apps_instalados jsonb DEFAULT '[]'::jsonb, p_pin text DEFAULT NULL::text, p_tipo_da_licenca text DEFAULT NULL::text, p_empresa uuid DEFAULT NULL::uuid, p_grupo_dispositivos text DEFAULT NULL::text, p_campanhas jsonb DEFAULT '[]'::jsonb, p_ip_dispositivo text DEFAULT NULL::text, p_num_filial text DEFAULT NULL::text, p_online boolean DEFAULT true, p_device_type text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF v_device_id IS NULL THEN
    RAISE EXCEPTION 'dispositivo não cadastrado!'
      USING ERRCODE = 'P0002', HINT = 'device_not_registered';
  END IF;
