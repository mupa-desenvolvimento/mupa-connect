-- Update the existing RPC function to return the persistence field
...
BEGIN
  ...
  -- 6. Preparar retorno completo incluindo o campo persistence
...
-- Function to handle persistence logic when a device goes offline
CREATE OR REPLACE FUNCTION public.handle_device_persistence()
RETURNS TRIGGER AS $$
BEGIN
  -- If persistence is active and the device just went offline
  IF NEW.persistence = true AND NEW.online = false AND (OLD.online = true OR OLD.online IS NULL) THEN
    -- Send command to open the specific app
    NEW.comando := 'open_app:com.mupa.apptc';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to execute the persistence logic
DROP TRIGGER IF EXISTS trigger_device_persistence ON public.dispositivos;
CREATE TRIGGER trigger_device_persistence
BEFORE UPDATE OF online, persistence ON public.dispositivos
FOR EACH ROW
EXECUTE FUNCTION public.handle_device_persistence();
