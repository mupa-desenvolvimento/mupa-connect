-- First, drop the conflicting function I created earlier
DROP FUNCTION IF EXISTS public.get_groups_hierarchy(UUID);

-- Restore the original get_groups_hierarchy function that the UI expects
...
-- Update the get_group_path function (json version for path only)
CREATE OR REPLACE FUNCTION public.get_group_path_json(group_uuid UUID)
RETURNS JSONB AS $$
DECLARE
    hierarchy JSONB;
BEGIN
