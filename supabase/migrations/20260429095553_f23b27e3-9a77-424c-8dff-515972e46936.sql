...
BEGIN
    ...
    device_nodes AS (
        ...
        WHERE d.empresa = '1728965891007x215886838679286700'
    ),
    all_combined AS (
    ...
    FROM (
        SELECT 
            ...
            CASE 
                ...
                WHEN ac.type = 'store' THEN (SELECT count(*) FROM public.dispositivos d WHERE d.num_filial::text = (SELECT code FROM public.stores WHERE id = ac.id))
                ELSE (SELECT count(*) FROM public.dispositivos d WHERE d.empresa = '1728965891007x215886838679286700')
            END as device_count
