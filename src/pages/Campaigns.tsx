            ...
            {view === "grid" && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 overflow-y-auto h-full pr-2">
                {filteredCampaigns.map((c) => (
                  <Card key={c.id} className="group border-border/60 shadow-sm hover:shadow-md transition-all">
                    <div className="h-2 w-full rounded-t-lg" style={{ backgroundColor: c.color || '#9b87f5' }} />
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleEdit(c.id)}><Edit2 className="h-4 w-4 mr-2" /> Editar</DropdownMenuItem>
                            <DropdownMenuItem className="text-destructive" onClick={() => handleDelete(c.id)}><Trash2 className="h-4 w-4 mr-2" /> Excluir</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                      <div className="text-xs text-muted-foreground space-y-1">
                        <p>{format(new Date(c.start_date), "dd/MM")} - {format(new Date(c.end_date), "dd/MM")}</p>
                        <p>{c.start_time.substring(0,5)} até {c.end_time.substring(0,5)}</p>
                      </div>
                      <div className="flex justify-between items-center mt-2 border-t pt-2">
                        <Badge variant="outline" className="text-[10px] font-bold">P{c.priority}</Badge>
                    ...
                    {filteredCampaigns.map(c => (
                      <tr key={c.id} className="hover:bg-muted/10">
                        <td className="px-4 py-3 font-bold flex items-center gap-2">
                          <div className="h-8 w-1.5 rounded-full" style={{ backgroundColor: c.color }} />
                          {c.name}
                        </td>
